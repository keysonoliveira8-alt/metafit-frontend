...
import {
  ...
  Circle, RotateCcw, Info, Zap, Award, Calendar as CalendarIcon, Bell, FileText, LogOut
} from "lucide-react";
...
/* ============================== WORKOUTS ============================== */
function WorkoutScreen({ profile, todayWorkout, setTodayWorkout, history, weekCompleted, onOpenSuggest, showToast, onWorkoutCompleted }) {
      ...
      {showFeedback && (
        <WorkoutFeedbackModal
          ...
          onSubmit={(fb) => {
            ...
            onWorkoutCompleted({ id: "w" + Date.now(), date: "Hoje", name: todayWorkout.muscleGroup, duration: durationMin, sets: totalSetsDone, ...fb });
...
/* ============================== EVOLUTION ============================== */
function EvolutionScreen({ profile, weightHistory, onAddWeight, measurements, setMeasurements, goals, weekCompleted }) {
...
/* ============================== PROFILE ============================== */
function ProfileScreen({ profile, onOpenPremium, onRestartOnboarding, onLogout }) {
  const rows = [
  ...
  return (
    <div style={{ padding: "20px 16px 100px" }}>
      ...
      <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <Button variant="ghost" full onClick={onLogout}><LogOut size={16} /> Sair da conta</Button>
      </div>
    </div>
  );
}
...
const emptyMeals = { "Café da manhã": [], "Almoço": [], "Jantar": [], "Lanches": [] };
const todayISO = () => new Date().toISOString().slice(0, 10);

function profileRowToState(row) {
  return {
    name: row.nome, age: row.idade, sex: row.sexo, height: row.altura, weight: row.peso, weightGoal: row.peso_meta,
    goal: row.objetivo, activityLevel: row.nivel_atividade, trainDays: row.dias_treino, trainTime: row.tempo_treino,
    location: row.local_treino, equipment: row.equipamentos, dailyCalories: row.meta_calorias,
    proteinGoal: row.meta_proteina, carbsGoal: row.meta_carboidrato, fatGoal: row.meta_gordura,
  };
}
function profileStateToRow(p, userId) {
  return {
    user_id: userId, nome: p.name, idade: p.age, sexo: p.sex, altura: p.height, peso: p.weight, peso_meta: p.weightGoal,
    objetivo: p.goal, nivel_atividade: p.activityLevel, dias_treino: p.trainDays, tempo_treino: p.trainTime,
    local_treino: p.location, equipamentos: p.equipment, meta_calorias: p.dailyCalories,
    meta_proteina: p.proteinGoal, meta_carboidrato: p.carbsGoal, meta_gordura: p.fatGoal,
  };
}
...
export default function App() {
  ...
  const loadedRef = useRef(false); // evita re-sincronizar com o banco logo apos carregar os dados
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: perfilRow } = await supabase.from("perfis").select("*").eq("user_id", userId).maybeSingle();
      ...
      setProfile(profileRowToState(perfilRow));
      setOnboarded(true);

      const [{ data: refeicoesRows }, { data: treinosRows }, { data: pesoRows }, { data: medidasRows }, { data: metasRows }] = await Promise.all([
        supabase.from("refeicoes").select("*").eq("user_id", userId).eq("data", todayISO()),
        supabase.from("treinos").select("*").eq("user_id", userId).order("data", { ascending: false }),
        supabase.from("peso_historico").select("*").eq("user_id", userId).order("data", { ascending: true }),
        supabase.from("medidas").select("*").eq("user_id", userId).order("data", { ascending: false }).limit(1),
        supabase.from("metas").select("*").eq("user_id", userId),
      ]);

      const mealsState = { ...emptyMeals };
      (refeicoesRows || []).forEach((r) => {
        if (!mealsState[r.categoria]) mealsState[r.categoria] = [];
        mealsState[r.categoria].push({ id: r.id, name: r.nome, time: r.horario, calories: r.calorias, protein: r.proteina, carbs: r.carboidrato, fat: r.gordura });
      });
      setMeals(mealsState);

      const planejado = (treinosRows || []).find((t) => t.status === "planejado" && t.data === todayISO());
      setTodayWorkout(planejado ? { name: planejado.nome, muscleGroup: planejado.grupo_muscular, durationEstimate: planejado.duracao_estimada, exercises: planejado.exercicios } : null);

      const concluidos = (treinosRows || []).filter((t) => t.status === "concluido");
      setWorkoutHistory(concluidos.map((t) => ({ id: t.id, date: t.data, name: t.grupo_muscular, duration: t.duracao_real, sets: (t.exercicios || []).reduce((a, e) => a + (e.sets || 0), 0), feeling: t.sensacao, difficulty: t.dificuldade, notes: t.observacoes })));
      const seteDiasAtras = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10);
      setWeekCompleted(concluidos.filter((t) => t.data >= seteDiasAtras).length);

      if (pesoRows && pesoRows.length > 0) {
        setWeightHistory(pesoRows.map((r) => ({ date: r.data, weight: Number(r.peso) })));
      } else {
        setWeightHistory([{ date: todayISO(), weight: profileRowToState(perfilRow).weight }]);
      }

      if (medidasRows && medidasRows.length > 0) {
        const m = medidasRows[0];
        setMeasurements({
          current: { braço: m.braco, peito: m.peito, cintura: m.cintura, quadril: m.quadril, coxa: m.coxa, panturrilha: m.panturrilha },
          initial: { braço: m.braco, peito: m.peito, cintura: m.cintura, quadril: m.quadril, coxa: m.coxa, panturrilha: m.panturrilha },
        });
      }

      if (metasRows && metasRows.length > 0) {
        setGoals(metasRows.map((g) => ({ id: g.id, label: g.label, current: Number(g.atual), target: Number(g.alvo), unit: g.unidade })));
      }
  ...
  // sincroniza refeicoes de hoje com o banco quando mudam
  useEffect(() => {
    if (!session || !loadedRef.current) return;
    const userId = session.user.id;
    (async () => {
      await supabase.from("refeicoes").delete().eq("user_id", userId).eq("data", todayISO());
      const rows = Object.entries(meals).flatMap(([categoria, items]) =>
        items.map((it) => ({ user_id: userId, categoria, nome: it.name, horario: it.time, calorias: it.calories, proteina: it.protein, carboidrato: it.carbs, gordura: it.fat, data: todayISO() }))
      );
      if (rows.length > 0) await supabase.from("refeicoes").insert(rows);
    })();
  }, [meals]);

  // sincroniza o treino planejado de hoje com o banco quando muda
  useEffect(() => {
    if (!session || !loadedRef.current) return;
    const userId = session.user.id;
    (async () => {
      await supabase.from("treinos").delete().eq("user_id", userId).eq("status", "planejado").eq("data", todayISO());
      if (todayWorkout) {
        await supabase.from("treinos").insert({
          user_id: userId, nome: todayWorkout.name, grupo_muscular: todayWorkout.muscleGroup,
          exercicios: todayWorkout.exercises, duracao_estimada: todayWorkout.durationEstimate, status: "planejado", data: todayISO(),
        });
      }
    })();
  }, [todayWorkout]);

  const handleWorkoutCompleted = async (entry) => {
    setWorkoutHistory((h) => [...h, entry]);
    ...
    if (session) {
      ...
      await supabase.from("treinos").insert({
        user_id: userId, nome: todayWorkout.name, grupo_muscular: todayWorkout.muscleGroup,
        exercicios: todayWorkout.exercises, duracao_estimada: todayWorkout.durationEstimate,
        status: "concluido", duracao_real: entry.duration, dificuldade: entry.difficulty, sensacao: entry.feeling, observacoes: entry.notes, data: todayISO(),
      });
    }
  };

  const handleAddWeight = async (weight) => {
    setWeightHistory((h) => [...h, { date: todayISO(), weight }]);
    if (session) {
      await supabase.from("peso_historico").insert({ user_id: session.user.id, peso: weight, data: todayISO() });
      await supabase.from("perfis").update({ peso: weight }).eq("user_id", session.user.id);
    }
  };
  ...
  const handleOnboardingComplete = async (p) => {
    ...
    setWorkoutHistory([]);
    setWeekCompleted(0);
    setWeightHistory([{ date: todayISO(), weight: p.weight }]);
    if (session) {
      const userId = session.user.id;
      await supabase.from("perfis").upsert(profileStateToRow(p, userId));
      await supabase.from("peso_historico").insert({ user_id: userId, peso: p.weight, data: todayISO() });
        ...
        <>
          ...
          {tab === "workout" && (
            <WorkoutScreen profile={profile} todayWorkout={todayWorkout} setTodayWorkout={setTodayWorkout}
              history={workoutHistory} weekCompleted={weekCompleted}
              onOpenSuggest={() => setShowAISuggestWorkout(true)} showToast={showToast} onWorkoutCompleted={handleWorkoutCompleted} />
          )}
          {tab === "evolution" && (
            <EvolutionScreen profile={profile} weightHistory={weightHistory} onAddWeight={handleAddWeight}
              measurements={measurements} setMeasurements={setMeasurements} goals={goals} weekCompleted={weekCompleted} />
          )}
          {tab === "profile" && (
            <ProfileScreen profile={profile} onOpenPremium={() => setShowPremium(true)} onRestartOnboarding={() => setOnboarded(false)} onLogout={handleLogout} />
          )}
